...
rem   2. commit outputs/predictions/ (only that folder, and only if something changed)
...
git diff --cached --quiet -- outputs/predictions
if not errorlevel 1 (
...
git commit -q -m "predictions: refresh scheduled cards (%STAMP%)" -- outputs/predictions
if errorlevel 1 (
