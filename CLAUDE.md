...
- **All scheduled cards** (`run_predict_all`, `python run_pipeline.py --predict-all`): refresh the ufc.com schedule (on failure log it and use the existing files), then predict every card file in `data/raw/cards/` dated today or later (cards without announced bouts are listed but not predicted), writing each card's CSV + standalone HTML (archive copy, linking back to `index.html#<event_slug>`) and `outputs/predictions/index.html`: a single self-contained dashboard with every scheduled card. Sticky top bar with one tab per event (short title + date badge, e.g. "UFC 332 (Oct 3)"; a `<select>` on narrow screens), one `<section class="event-panel" id="<event_slug>">` per event with all its bout cards (same bars, pick/debut badges and key-factor tags as the card report), `#<event_slug>` deep links, default = the next card with bouts. A separate first tab, "Card changes" (`#card-changes`, with a count badge), lists every line-up change across all events (newest first, with the detection date/time in UTC and the event, as replacement / new bout / off the card) and when the cards were last checked with ufc.com; there is no changes box above the event panels. Each event panel shows its own change history and check time, and tabs with a change in the last 7 days get a dot. Bouts in each card (dashboard and standalone page) are grouped under Main card / Prelims / Early prelims headings from the ufc.com page section (`card_segment` in the card JSON; cards without it, e.g. manual ones, show one grid). Changes are dated when *we* detected them, not when the UFC announced them. Exception to the no-scripts rule for this file only: one small inline vanilla-JS tab switcher; without JavaScript all events show stacked and the tabs are plain anchors. The only external script allowed anywhere is the optional GoatCounter visit counter (approved 2026-09-26; cookie-free, no personal data): when `analytics.goatcounter_code` is set, `index.html` loads `https://gc.zgo.at/count.js` for `https://<code>.goatcounter.com`, the tab switcher counts user tab switches as events (`tab/<slug>`), and the footer says so. Empty code = no analytics at all. The page must work when the script is blocked (tested offline in headless Edge/Chrome). Standalone card pages stay script-free. A test runs the page in headless Edge/Chrome (skipped if none is installed) and checks the script's `data-dashboard="ready"` marker.
