...
def build_history(tables: dict[str, pd.DataFrame]) -> History:
    ...
    return History(states=states, weight_classes=wc, fighters=fighters)
...
def fighter_features(sides: pd.DataFrame, history: History) -> pd.DataFrame:
    ...
    if (st["hist_date"] >= st["event_date"]).any():
        raise AssertionError("leakage: history state dated on/after the fight date")
    ...
    out["_hist_date"] = st["hist_date"].to_numpy()
    return out


def matchup_features(pairs: pd.DataFrame, history: History) -> pd.DataFrame:
    ...
    ff = fighter_features(sides.drop(columns="side"), history).set_index("row_id")
    ...
    for side, part in parts.items():
        ...
        cols[f"_f{side}_hist_date"] = part["_hist_date"]
    for f in FIGHTER_FEATURES:
...
def build_training_features(tables: dict[str, pd.DataFrame], split_cfg: dict,
                            history: History | None = None) -> pd.DataFrame:
    ...
    feats = matchup_features(both, history)
    feats = feats.assign(split=assign_split(feats["event_date"], split_cfg))
    cols = ID_COLUMNS + CATEGORICAL + numeric_feature_columns() + ["_f1_hist_date", "_f2_hist_date"]
...
def run_features() -> pd.DataFrame:
    ...
    # Belt and braces: every used history state is strictly before the fight.
    for c in ("_f1_hist_date", "_f2_hist_date"):
        if (feats[c].notna() & (feats[c] >= feats["event_date"])).any():
            raise AssertionError(f"leakage in {c}")
    out = feats.drop(columns=["_f1_hist_date", "_f2_hist_date"])
    ...
    (out_dir / "feature_list.json").write_text(json.dumps({
        ...
        "fighter_features": FIGHTER_FEATURES}, indent=2), encoding="utf-8")
